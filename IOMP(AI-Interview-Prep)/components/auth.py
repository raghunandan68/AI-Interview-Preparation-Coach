...
class GuestUser:
    def __init__(self):
        ...
        self.user_metadata = {'full_name': 'Guest'}
        self.is_dev = True
...
def render_auth():
    ...
    with col2:
        ...
        if st.button("🚀 Guest Mode: Bypass Login", use_container_width=True, type="secondary"):
            # Note: We no longer need to upsert the guest user to Supabase
            # because history is not saved in Guest Mode.
            guest = GuestUser()
            
            st.session_state.authenticated = True
            st.session_state.user_data = guest
