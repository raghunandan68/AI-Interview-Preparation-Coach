...
def inject_tab_switching_js(remaining_seconds=None):
    ...
    // Also listen in the iframe window
    ...
    // Detect exit from fullscreen (often via Esc)
    ...
    // 3. Hide the secret buttons visually using JS
    setTimeout(() => {{
        const buttons = Array.from(parentDoc.querySelectorAll('button'));
        ...
        // 4. Fullscreen logic (Auto-fullscreen attempt)
